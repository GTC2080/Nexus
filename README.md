...
- **v1.0.6** — 完成 15 项性能优化，并补齐 `VectorCacheState` 的 top-k 堆顺序与缓存生命周期修复；覆盖保存队列 Map 化、增量 watcher、向量内存缓存、PDF 去 base64、拖拽零重渲染、图谱/文件树指纹修正
...
- **全量性能优化（v1.0.6）**：15 项优化覆盖 P0-P3，并补齐 `VectorCacheState` 的 top-k 堆顺序与 `upsert / remove / clear` 生命周期同步。保存队列 Map 化消除多文件丢数据；增量监听链路替代全库扫描；向量检索内存缓存 + top-k BinaryHeap；PDF 去 base64 + 预取限流；面板拖拽 CSS var 零渲染；图谱/文件树 FNV-1a 指纹修正；性能基线工具与 bundle 分析流程
