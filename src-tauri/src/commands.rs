...
/// 允许进行 AI 向量化的扩展名
pub(super) const EMBEDDABLE_EXTENSIONS: &[&str] = &["md", "txt", "pdf"];

pub(super) fn read_ai_config(app: &AppHandle) -> Result<AiConfig, String> {
    let store = app
        .store("settings.json")
...
pub(super) fn is_embeddable_extension(ext: &str) -> bool {
    EMBEDDABLE_EXTENSIONS.iter().any(|e| e.eq_ignore_ascii_case(ext))
}

pub(super) fn is_canvas_extension(ext: &str) -> bool {
    ext.eq_ignore_ascii_case("canvas")
}

pub(super) fn is_pdf_extension(ext: &str) -> bool {
    ext.eq_ignore_ascii_case("pdf")
}

pub(super) fn is_timeline_extension(ext: &str) -> bool {
    ext.eq_ignore_ascii_case("timeline")
}

pub(super) fn is_spectroscopy_extension(ext: &str) -> bool {
    ext.eq_ignore_ascii_case("csv") || ext.eq_ignore_ascii_case("jdx")
}

pub(super) fn semantic_candidate_limit(limit: usize) -> usize {
    (limit.saturating_mul(40)).clamp(200, 2000)
}

/// 从 PDF 文件中提取纯文本内容（在大栈线程中运行，防止栈溢出崩溃）
pub(super) fn extract_pdf_text(path: &Path) -> Result<String, String> {
